...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    LUInsetView *insetView = [[LUInsetView alloc] initWithFrame:self.bounds];
...
- (LUShader *) colorShader
{
    if (!colorShader_) {
        NSArray *attributes = @[@"inPosition", @"inTexcoord"];
        NSArray *uniforms = @[@"modelViewProjectionMatrix", @"hue"];
        
        colorShader_ = [[LUShader alloc] initWithVertexShader:@"blit" fragmentShader:@"colorPicker"
...
- (void) drawView
{
    ...
    // use shader program
    LUShader *colorShader = self.colorShader;
...
- (CGPoint) indicatorPosition
{
    ...
    result = LUAddPoints(result, self.trackRect.origin);
...
- (void) computeValueForTouch:(UITouch *)touch
{
    ...
    pt.x = LUClamp(CGRectGetMinX(self.trackRect), CGRectGetMaxX(self.trackRect), pt.x);
    pt.y = LUClamp(CGRectGetMinY(self.trackRect), CGRectGetMaxY(self.trackRect), pt.y);
