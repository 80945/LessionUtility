...
CGSize LUSizeOfRectWithAngle(CGRect rect, float angle, CGPoint *upperLeft, CGPoint *upperRight)
{
    ...
    if (upperLeft) {
        *upperLeft = LUSubtractPoints(corners[0], center);
    }
    
    if (upperRight) {
        *upperRight = LUSubtractPoints(corners[3], center);
    }
...
{
    float distance = LUDistance(CGPointZero, vector);
    
    if (distance == 0.0f) {
        return vector;
    }
    
    return LUMultiplyPointScalar(vector, 1.0f / LUDistance(CGPointZero, vector));
}
...
CGPoint LUSharpPointInContext(CGPoint pt, CGContextRef ctx)
{
    ...
    pt = LUFloorPoint(pt);
    pt = LUAddPoints(pt, CGPointMake(0.5f, 0.5f));
    pt = CGContextConvertPointToUserSpace(ctx, pt);
    
    return pt;
}

CGPoint LUConstrainPoint(CGPoint delta)
{
    float   angle = atan2(delta.y, delta.x);
    float   magnitude = LUDistance(delta, CGPointZero);
...
void convertQuadraticPathElement(void *info, const CGPathElement *element)
{
    ...
    switch (element->type) {
        ...
        case kCGPathElementAddQuadCurveToPoint:
            ...
            CGPoint outPoint = LUAddPoints(prev, LUMultiplyPointScalar(LUSubtractPoints(element->points[0], prev), 2.0f / 3));
            CGPoint inPoint = LUAddPoints(element->points[1], LUMultiplyPointScalar(LUSubtractPoints(element->points[0], element->points[1]), 2.0f / 3));
...
BOOL LUCollinear(CGPoint a, CGPoint b, CGPoint c)
{
    ...
    distances[0] = LUDistance(a, b);
    distances[1] = LUDistance(b, c);
    distances[2] = LUDistance(a, c);
...
BOOL LULineInRect(CGPoint a, CGPoint b, CGRect test)
{
    ...
    if(acode == 0 || bcode == 0) { // one or both endpoints within rect
        ...
        if (LULineInRect(a, middle, test)) {
            return YES;
        }
        
        if (LULineInRect(middle, b, test)) {
            return YES;
        }
        return NO;
    }
...
{
    LUPathAndTransform  pathAndTransform = *((LUPathAndTransform *) info);
...
CGPathRef LUTransformCGPathRef(CGPathRef pathRef, CGAffineTransform transform)
{
    ...
    LUPathAndTransform  pathAndTransform = {transformedPath, transform};
...
BOOL LULineSegmentsIntersectWithValues(CGPoint A, CGPoint B, CGPoint C, CGPoint D, float *rV, float *sV)
{
...
{
    return LULineSegmentsIntersectWithValues(A, B, C, D, NULL, NULL);
}
...
{
    return min + LURandomFloat() * (max - min);
}
...
LUQuad LUQuadNull()
{
    ...
    return LUQuadMake(bogusPoint, bogusPoint, bogusPoint, bogusPoint);
}
...
{
    LUQuad nullQuad = LUQuadNull();
    if (LUQuadEqualToQuad(a, nullQuad) || LUQuadEqualToQuad(b, nullQuad)) {
        return NO;
    }
    
    for (int i = 0; i < 4; i++) {
        for (int n = 0; n < 4; n++) {
            if (LULineSegmentsIntersect(a.corners[i], a.corners[(i+1)%4], b.corners[n], b.corners[(n+1)%4])) {
                return YES;
            }
        }
    }
...
void LUCheckGLError_(const char* file, int line) {
    ...
    if (error) {
        ...
        LULog(@"ERROR: glGetError returned: %@ at %s:%d", message, file, line);
    }
}
...
void LULogTiming(NSString *message)
{
    if (!timings_ || timings_.count == 0) {
        NSLog(@"LULogTiming() Error: unbalanced calls.");
        return;
    }
...
{
    if (!timings_ || timings_.count == 0) {
        NSLog(@"LUEndTiming() Error: unbalanced calls.");
        return;
    }
...
{
    return (LUDeviceIsPhone() && [UIScreen mainScreen].bounds.size.height == 568) ? YES : NO;
}
...
BOOL LUCanUseScissorTest()
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
            ...
            CGSize size = LUMultiplySizeScalar([UIScreen mainScreen].bounds.size, [UIScreen mainScreen].scale);
